...
const getSelectedExpenses = (expenses, filters) => {
    return expenses.filter((expense) => {
        ...
        const dateFromMatch = moment(expense.date, 'D/MM/YYYY').isAfter(filters.startDate);
        const dateToMatch = moment(expense.date, 'D/MM/YYYY').isBefore(filters.endDate);

        return categoryMatch && amountFromMatch && amountToMatch && dateFromMatch && dateToMatch;
